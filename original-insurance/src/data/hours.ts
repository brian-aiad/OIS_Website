/**
 * Office hours data for Original Insurance Services.
 *
...
export const REGULAR_HOURS = {
  weekdays: { opens: "10:00", closes: "17:30" },
  saturday: null,
  sunday: null,
};
...
export function getOfficeStatus(): { open: boolean; label: string; holidayWarning?: string } {
  ...
  const isOpen = isWeekday && decimal >= 10 && decimal < 17.5;

  if (isOpen) return { open: true, label: "Open Now", holidayWarning };
  if (day === 0 || day === 6) return { open: false, label: "Opens Mon 10:00 AM", holidayWarning };
